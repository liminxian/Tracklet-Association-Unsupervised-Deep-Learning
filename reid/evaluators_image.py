...
def extract_features(model, data_loader, print_freq=1, metric=None):
    ...
    for i, (imgs, fnames, tids, pids, tids_percam, pids_percam, cams) in enumerate(data_loader):
        data_time.update(time.time() - end)

        outputs = extract_cnn_feature(model, imgs, cams)
        print("size of outputs: ", outputs.__sizeof__())
